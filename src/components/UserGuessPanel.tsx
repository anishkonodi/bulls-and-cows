...
const UserGuessPanel: React.FC<UserGuessPanelProps> = ({ onGuess, disabled, lastResult }) => {
  ...
  return (
    <div className={`p-3 sm:p-6 rounded-xl border-2 transition-all flex flex-col h-full ${disabled ? 'bg-slate-50 border-slate-100 opacity-75' : 'bg-white border-indigo-100 shadow-md shadow-indigo-50'}`}>
      ...
      <p className="hidden sm:block text-slate-500 text-sm mb-4 sm:mb-6 shrink-0">Guess the system's secret 4-digit number.</p>

      <form onSubmit={handleSubmit} className="flex flex-col gap-2 sm:gap-4 flex-1">
        <div className="flex-1">
          <label htmlFor="user-guess" className="hidden sm:block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
            Enter 4-Digit Guess
          </label>
          <input
            id="user-guess"
            ...
            className="w-full px-3 py-2 sm:px-4 sm:py-3 rounded-lg border border-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all font-mono text-lg sm:text-xl tracking-[0.25em] sm:tracking-[0.5em] text-center"
          />
        ...
        <button
          ...
          className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white font-bold py-2 sm:py-3 rounded-lg transition-colors shadow-sm shadow-indigo-100 uppercase tracking-wide text-xs sm:text-sm shrink-0"
        >
          Submit Guess
        </button>
      </form>
      ...
      {lastResult && (
        <div className="hidden sm:flex mt-auto pt-6 p-4 bg-indigo-50 rounded-lg border border-indigo-100 justify-around shrink-0">
